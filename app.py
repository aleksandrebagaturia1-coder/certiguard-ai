...
Analyze the provided document text and look for structural anomalies, missing critical fields, or signs of fraud. 

CRITICAL INSTRUCTION: You MUST include one of the following exact tags anywhere in your response based on your final assessment:
- Use [RED] if the document has VOID marks, is missing crucial approval numbers/signatures, or has signs of fraud.
- Use [YELLOW] if there are minor missing non-critical fields but the document looks mostly authentic.
- Use [GREEN] if the document appears structurally perfect and ready for use.

Do NOT use markdown stars or bold text markers.
"""
...
with col2:
    ...
    if report:
        ...
        clean_display = report.replace("STATUS: RED", "").replace("STATUS: YELLOW", "").replace("STATUS: GREEN", "").replace("[RED]", "").replace("[YELLOW]", "").replace("[GREEN]", "").strip()
